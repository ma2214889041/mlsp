...
async def admin_update_inventory(
    ...
    try:
        ...
        prod = product_crud.get_product(db, product_id)
        if not prod:
            return RedirectResponse(url="/admin/inventory?error=product_not_found", status_code=303)
        ...
        return RedirectResponse(url=f"/admin/inventory?error={str(e)}", status_code=303)
...
async def admin_add_product_post(
    ...
    return RedirectResponse(url="/mlsp/admin/products", status_code=303)
...
async def admin_add_employee(
    ...
    if existing_employee:
        ...
        return RedirectResponse(url="/admin/employees?error=username_exists", status_code=303)
...
async def admin_edit_employee(
    ...
    if username != existing_employee.username:
        ...
        if username_check and username_check.id != employee_id:
            # 用户名已被使用
            return RedirectResponse(url=f"/admin/employees?error=username_exists&id={employee_id}", status_code=303)
