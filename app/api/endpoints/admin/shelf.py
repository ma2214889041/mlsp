...
async def add_shelf(request: Request, name: str = Form(...), location: str = Form(...),
    ...
    return RedirectResponse(url=f"/admin/shelf/{new_shelf.id}", status_code=303)
...
async def view_shelf(request: Request, shelf_id: int, db: Session = Depends(get_db),
    ...
    <html>
      <body>
        ...
        <p>二维码: /static/qrcodes/shelves/shelf_{shelf_obj.id}.png</p>
        <h3>货位列表</h3>
        <ul>
    """
    for slot in shelf_slots:
        html_content += f"<li>货位ID: {slot.id}，标识: {slot.position}，二维码: /static/qrcodes/slots/shelf_{shelf_obj.id}_slot_{slot.id}.png</li>"
    html_content += """
        </ul>
...
async def shelf_slot_add_form(request: Request, shelf_id: int, admin: bool = Depends(get_current_admin)):
    ...
    <html>
      <body>
        ...
        <form action="/admin/shelf/{shelf_id}/slot/add" method="post">
...
async def add_shelf_slot(request: Request, shelf_id: int, position: str = Form(...),
    ...
    return RedirectResponse(url=f"/admin/shelf/{shelf_id}", status_code=303)
...
async def delete_shelf(request: Request, shelf_id: int, db: Session = Depends(get_db),
    ...
    return RedirectResponse(url="/admin/shelves", status_code=303)
...
async def list_shelves(request: Request, db: Session = Depends(get_db),
    ...
    for s in shelves_list:
        html_content += f'<li><a href="/admin/shelf/{s.id}">{s.name}</a></li>'
