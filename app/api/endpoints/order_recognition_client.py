...
async def upload_order_photo(
    ...
    try:
        ...
        # --- MODIFICATION START: Return ALL available products for simulation ---
        # 模拟AI识别结果 - 返回所有可用产品，随机数量
        product_service = ProductService(db)
        products = product_service.get_available_products()

        products_result = []
        for product in products:
            quantity = random.randint(1, 3) # Give each product a random initial quantity
        ...
        active_sessions[session_id] = {
            ...
            "products": products_result, # Use the potentially longer list
        ...
        return JSONResponse(content={
            ...
            "photo_url": f"/static/uploads/order_photos/{filename}",
            "products": products_result, # Return the potentially longer list
            "message": "照片上传成功并已识别订单内容 (模拟返回所有产品)"
        })
